...
- 发现 `build: .` 配置
- 在当前目录查找 `Dockerfile`
- 使用 `Dockerfile` 构建镜像 `exchangeapp-backend:v1.0`
- 可以在命令后加服务名，只操作某一个服务（详见下方"指定服务名操作单个服务"章节）
### 运行阶段
```bash
docker-compose up -d
```
- 使用已构建的镜像 `exchangeapp-backend:v1.0`
...
```yaml
name: exchangeapp-backend # docker-compose 项目组名称
services:
  app:
...
```bash
# 停止并删除容器
docker stop exchangeapp-backend && docker rm exchangeapp-backend
